...
str(test_dataset)
# Есть текстовая (var12), но судя по значениям, там должно быть число
# Остальные переменные double, то есть число со знаком после запятой
# Не очень понимаю вопрос про отличие... Память выделяема под них отличается,
# функции, которые определены для типов тоже отличаются и механизм их действия
...
is_na_table <- is.na(test_dataset)
...
cleaned_dataset <- drop_na(test_dataset)
# Строк осталось мало... В 4 раза кропнули
...
final_columns <- test_dataset %>%
  select(var36, var37, var38) %>%
...
sorted_dataset <- renamed_dataset %>%
...
test_vec <- c(10, 20, 25, 60, 70, 42, 13, 100)

recalculate <- function(vector, reference_value) {
  100 - abs(reference_value - vector)
}

test_vec <- c(10, 20, 25, 60, 70, 42, 13, 100)
result <- recalculate(test_vec, 60)
print(result)
...
# Write the renamed dataset to result.csv
write_csv(renamed_dataset, "result.csv")
...
# Создаем функцию для генерации персонажа D&D
generate_character <- function() {
  # Шаг 1: Случайный выбор расы
  races <- c("Дварф", "Эльф", "Хафлинг", "Человек")
  race <- sample(races, 1)
  
  # Шаг 2: Случайный выбор класса
  classes <- c("Друид", "Воин", "Лучник", "Волшебник")
  class <- sample(classes, 1)
  
  # Шаг 3: Случайный выбор предыстории
  backgrounds <- c("Шарлатан", "Преступник", "Артист", "Народный герой", 
                   "Глава гильдии", "Отшельник", "Знать", "Чужестранец", 
                   "Мудрец", "Моряк", "Солдат", "Священник")
  background <- sample(backgrounds, 1)
  
  # Шаг 4: Генерация 6 характеристик
  roll_stat <- function() {
    rolls <- sample(1:6, 4, replace = TRUE)
    sorted_rolls <- sort(rolls, decreasing = TRUE)
    return(sum(sorted_rolls[1:3])) # Берем три наибольших значения и суммируем
  }
  
  attributes <- c("Сила", "Ловкость", "Телосложение", "Интеллект", "Мудрость", "Харизма")
  stats <- sapply(attributes, function(x) roll_stat())
  
  # Создаем фрейм для вывода персонажа
  character <- data.frame(
    "Атрибут" = c("Раса", "Класс", "Предыстория", attributes),
    "Значение" = c(race, class, background, stats)
  )
